...
def default_root() -> Path:
    ...
    if (package_root / "schemas").exists():
        return package_root
    distribution_root = package_root.parent
    if (distribution_root / "schemas").exists():
...
def cmd_preflight(args: argparse.Namespace) -> int:
    ...
    profile = load_structured(root / "profiles" / f"{volume['release_profile']}.yaml")
    compiled = compile_volume(volume, profile)
    issue_count = len(volume.get("issues", []))
    expected_page_counts = {4, 48}
    function_order_valid = compiled["page_count"] in expected_page_counts and (
        compiled["page_count"] == issue_count * 4
    )
    report = {
        "status": "PASS" if function_order_valid else "FAIL",
        "volume_id": volume["volume_id"],
        "state": volume["state"],
        "render_authorized": compiled["render_authorized"],
        "page_count": compiled["page_count"],
        "issue_count": issue_count,
        "expected_page_counts": sorted(expected_page_counts),
